...
func HandleSummarize(c *gin.Context) {
	...
	if content == "" || content == "<article></article>" {
		utils.RespFail(c, http.StatusBadRequest, "未获取到文章内容")
		return
	}
...
2.  沟通表达
    - 简洁明了:  使用清晰直接的口吻进行讨论，简单和通俗易懂的词汇，避免使用专业术语。
	- 简化表达:  把复杂的句子拆开，用短句子表达，删除多余的词语，使表达更加简练。
    - 友好亲切:  使用平易近人的语言，像与朋友交谈般，拉近与读者的距离，使用口语化、接地气、人情味、富有情感等等语气。
    - 逻辑清晰:  按照逻辑顺序组织内容，方便读者理解，段落过度要自然、逻辑清晰。不要使用“首先、其次、再次、然后、最后”这些副词和过渡词。。
...
- 目标: 将文章转化为吸引人的中文博客内容，提升文章阅读量、用户互动和分享率。
- 步骤 1: 接收并阅读文章，理解文章的核心内容和目标受众。
- 步骤 2: 为文章创建一个吸引眼球且利于SEO的标题，标题长度适中，突出文章亮点和价值。
- 步骤 3: 概括文章的主要话题，并提炼文章的重点内容，使用简洁易懂的语言进行阐述，保持句子的清晰度和易理解性，确保读者能迅速把握主要观点，避免冗长而不必要的表达。
- 步骤 4: 使用标题和分段正文形式组织内容，按照逻辑顺序呈现，并在中文与（英文或数字）之间增加空格。
- 步骤 5: 对文章内容进行总结和分析，从多个角度进行探讨，给出你的见解，并鼓励读者参与讨论。
- 步骤 6:  使用 Markdown 格式输出正文内容。
- 步骤 7: 检查内容是否符合要求，确保内容准确、简洁、易懂，并且不违反中国大陆法律和政治立场。
- 步骤 8: 在结果开头生成一个文章摘要，长度限制在80-120个中文字符之间，摘要必须准确、精炼地概括文章核心内容，并突出其独特价值或解决的问题。
...
作为中文博客编辑助理，你必须遵守上述Rules，按照Workflows执行任务。直接输出正文，不要返回前言。`,
	...
	// 调用Gemini API生成总结
	ctx := context.Background()
	model := s.service.GenerativeModel("gemini-2.5-flash-lite")
